<template >
  <v-hover v-slot="{hover}" :value="false">
    <v-card  :ripple="false"  @dblclick="changeContent = !changeContent" :color="selectedColor" :dark="themeColorActive == 'dark' ? true : false" min-width="200" width="280"  max-height="auto" :elevation="hover ? 7 : 2" :class="{ 'on-hover': hover } "  class="ma-5 cursor-pointer rounded-lg relative opacity-100">
      <v-card-text class="pa-0 inline-block overflow-hidden w-full">
       <v-img :src="`${task.Background}` || '' " :lazy-src="`${task.Background}` || '' " max-height="180" max-width="500" class="opacity-100"></v-img>
      </v-card-text>
      <v-list-item inactive :light="themeColorActive == 'light' ? true : false">
              <v-list-item-content class="z-50 pa-0">
                <v-form lazy-validation ref="myForm" class="pa-0" > 
                 ...
                 <v-text-field background-color="transparent" :value="task.title"  solo flat :autofocus="false" :rules="[rules.required , rules.TitleTextlenght]" :disabled="!changeContent"></v-text-field>

                </v-list-item-title>
                
                  <v-list-item-subtitle>
                    <v-text-field background-color="transparent" :value="task.description"  solo flat :rules="[rules.required , rules.DescriptionTextlenght]" :disabled="!changeContent" @keydown.enter="openDialog('change')"> </v-text-field>
                  </v-list-item-subtitle>
                </v-form>
            ...
            <v-list-item-action  class="w-full absolute  -bottom-1 -right-12 z-50 height-auto mt-10">
          
                  <div class="flex items-end justify-center mt-40" >
                              <v-btn
                                :class="{ 'show-btns': hover, 'darkTheme' :themeColorActive == 'dark'   }"
                                color="transparent"
                                icon
                                @click="changeStats"
                                title="Change Stats"
                              ...
                              >
                                ...
                                title="Delete"
                              >
                                <v-file-input accept="image/*"   hide-details hide-input :prepend-icon="hover ? 'mdi-image' : '' " @change="changeImage" v-model="image">
                      ...
                      <v-row class="absolute top-36 left-0 z-50" align="center" justify="center" v-if="ShowColorCanves" >
                        <v-col cols="12">
                          <v-color-picker v-model="colorSe" hide-mode-switch flat width="150" hide-inputs v-click-outside="onClickOutside" ></v-color-picker>
                        </v-col>
                      </v-row>              
